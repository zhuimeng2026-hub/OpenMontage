...
//
// Scoping: every store/usage key is the stable owner identity
// renderQueueOwnerID(sid) — a WeChat account keeps one identity across browser
// sessions and machines, so a template batch render (its metadata AND the
// upstream project that holds the uploaded images) is consistent for the same
// logged-in account on any device. Anonymous/dev flows fall back to the BFF
// session, preserving the previous per-device isolation.
...
func (h *TemplateHandler) CreateTemplate(c *gin.Context) {
	...
	if req.ProjectBase == "" {
		// Stable per account (scope is hex), so the upstream project bucket is
		// identical across the user's machines.
		req.ProjectBase = "ff-" + scope
	}
	if req.AspectRatio == "" {
	...
	t := h.Templates.SaveTemplate(scope, req.Name, req.ProjectBase, req.AspectRatio, req.TitleTemplate, req.DurationPerImage)
...
func (h *TemplateHandler) ListTemplates(c *gin.Context) {
	sid := h.ensureSession(c)
	scope := renderQueueOwnerID(sid)
	c.JSON(http.StatusOK, gin.H{"templates": h.Templates.ListTemplates(scope)})
}

func (h *TemplateHandler) GetTemplate(c *gin.Context) {
	sid := h.ensureSession(c)
	scope := renderQueueOwnerID(sid)
	t := h.Templates.GetTemplate(scope, c.Param("id"))
	if t == nil {
...
func (h *TemplateHandler) AddScenario(c *gin.Context) {
	...
	sc := h.Templates.AddScenario(scope, t.ID, req.BusinessKey)
...
func (h *TemplateHandler) ListScenarios(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"scenarios": h.Templates.ListScenarios(scope, t.ID)})
}
...
func (h *TemplateHandler) BatchRender(c *gin.Context) {
	...
	scenarios := h.Templates.ListScenarios(scope, t.ID)
	if len(scenarios) == 0 {
	...
	for _, sc := range scenarios {
		...
		h.Templates.SetScenarioImages(scope, sc.ID, imgs)
		totalFiles += len(imgs)
	}
	...
	snap, ok := h.Usage.Acquire(scope, tier, lim)
	if !ok {
	...
	job := h.Templates.CreateBatchJob(scope, t.ID, ids)
	...
	go func() {
		defer h.Usage.Release(scope)
		h.runBatch(scope, t, scenarios, job.ID)
	}()
...
func (h *TemplateHandler) GetBatchJob(c *gin.Context) {
	sid := h.ensureSession(c)
	scope := renderQueueOwnerID(sid)
	job := h.Templates.GetBatchJob(scope, c.Param("jobId"))
	if job == nil {
...
func (h *TemplateHandler) GetQuota(c *gin.Context) {
	...
	tier := h.Limits.Resolve(scope)
	lim := limits.ForTier(tier)
	snap := h.Usage.Inspect(scope, tier, lim)
...
//
// The scope is the stable owner identity (renderQueueOwnerID), so uploads land
// in the same upstream MCP session / project namespace the account uses on any
// machine, and the batch job + scenario state are visible to that account
// everywhere.
func (h *TemplateHandler) runBatch(scope string, t *template.Template, scenarios []*template.Scenario, jobID string) {
	ctx := context.Background()
	for _, sc := range scenarios {
		imgs := sc.ImageRefs
		if len(imgs) == 0 {
			h.Templates.SetScenarioStatus(scope, sc.ID, "failed", "no images resolved for scenario")
			h.Templates.SetJobOutput(scope, jobID, sc.ID, "")
			continue
		}
		h.Templates.SetScenarioStatus(scope, sc.ID, "uploading", "")
		projectID := t.ProjectBase + "-" + sc.ID
		uploadOK := true
		for _, img := range imgs {
			data, derr := downloadBytes(ctx, img.URL, img.Headers)
			if derr != nil {
				h.Templates.SetScenarioStatus(scope, sc.ID, "failed", "download "+img.URL+": "+derr.Error())
				uploadOK = false
				break
			}
			if uerr := uploadImageBytes(h.Sessions, scope, projectID, img.Name, data); uerr != nil {
				h.Templates.SetScenarioStatus(scope, sc.ID, "failed", "upload: "+uerr.Error())
				uploadOK = false
				break
			}
		}
		...
		h.Templates.SetScenarioStatus(scope, sc.ID, "rendering", "")
		...
		res, rerr := h.Sessions.Call(scope, "create_remotion_video_share", map[string]interface{}{
			"project_id":         projectID,
			"duration_per_image": t.DurationPerImage,
			"aspect_ratio":       t.AspectRatio,
			"title":              title,
			// scope IS the stable owner key; reuse it directly rather than
			// re-deriving it (avoids a double hash of an already-hashed value).
			"queue_owner_id": scope,
		})
		if rerr != nil {
			h.Templates.SetScenarioStatus(scope, sc.ID, "failed", "render submit: "+rerr.Error())
			h.Templates.SetJobOutput(scope, jobID, sc.ID, "")
			continue
		}
		renderJobID := digString(res, "render_job_id")
		if renderJobID == "" {
			h.Templates.SetScenarioStatus(scope, sc.ID, "failed", "no render_job_id in upstream response")
			h.Templates.SetJobOutput(scope, jobID, sc.ID, "")
			continue
		}
		h.Templates.SetScenarioRenderJob(scope, sc.ID, renderJobID)

		// Each batch-rendered scenario is also an entry in the caller's own
		// render queue (scoped by the stable owner identity, so never
		// cross-visible between accounts, yet consistent across the account's
		// own devices).
		jobStatus := "渲染中"
		...
		videoURL, perr := h.pollRender(scope, renderJobID)
		if perr != nil {
			h.Templates.SetScenarioStatus(scope, sc.ID, "failed", perr.Error())
			h.Templates.SetJobOutput(scope, jobID, sc.ID, "")
			continue
		}
		h.Templates.SetScenarioStatus(scope, sc.ID, "done", "")
		h.Templates.SetScenarioVideo(scope, sc.ID, videoURL)
		h.Templates.SetJobOutput(scope, jobID, sc.ID, videoURL)
	}
	h.Templates.SetJobDone(scope, jobID)
}
...
func (h *TemplateHandler) pollRender(scope, renderJobID string) (string, error) {
	const maxPolls = 180 // 180 * 5s = 15min ceiling
	for i := 0; i < maxPolls; i++ {
		res, err := h.Sessions.Call(scope, "get_render_status", map[string]interface{}{"render_job_id": renderJobID})
		if err != nil {
			return "", err
		}
...
// using the resumable chunk protocol (start -> append* -> complete). The scope
// is the stable owner identity so the bytes land in the account's upstream
// session regardless of which device submitted them.
func uploadImageBytes(s *mcp.SessionStore, scope, projectID, filename string, data []byte) error {
	sum := sha256.Sum256(data)
	sha := hex.EncodeToString(sum[:])
	uploadID := randHex(16)
	if _, err := s.Call(scope, "upload_asset_chunk", map[string]interface{}{
		"operation":   "start",
		"project_id":  projectID,
		"filename":    filename,
	...
	_, err := s.Call(scope, "upload_asset_chunk", map[string]interface{}{
		"operation":  "complete",
		"project_id": projectID,
		"filename":   filename,
		"upload_id":  uploadID,
