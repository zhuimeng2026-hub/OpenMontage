...
验收要求是 `frameflow-bff.service` 与 `openmontage-mcp.service` 为 active，8080 `/api/me` 返回 200，8900 `/mcp` 在无 token 时返回 401；若存在遗留 `mcp-server.service`，脚本会执行 `disable --now`。脚本不会打印环境变量或服务密钥。
