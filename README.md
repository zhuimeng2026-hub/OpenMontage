...
> **No `make`?** macOS/Linux: `python3 -m venv .venv && source .venv/bin/activate && python -m pip install -r requirements.txt && cd remotion-composer && npm install && cd .. && cd demo && npm install && cd .. && python -m pip install piper-tts && cp .env.example .env`
>
...
"Script mode" lets users submit custom Remotion TSX source (e.g. generated by
DeepSeek), compiled and rendered at runtime by
`remotion-composer/src/CustomComposition.tsx`. When generating or authoring a
script, follow this props contract exactly — these are the only inputs the
component injects into your code:
